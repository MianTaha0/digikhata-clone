...
interface ProductDao {
    ...
    @Query("SELECT * FROM products WHERE businessId = :bid ORDER BY name COLLATE NOCASE ASC")
    fun getByBusiness(bid: Long): Flow<List<Product>>

    @Query("SELECT COALESCE(SUM(quantity * costPrice), 0) FROM products WHERE businessId = :bid")
    fun totalValue(bid: Long): Flow<Double>

    @Query("SELECT COUNT(*) FROM products WHERE businessId = :bid AND lowStockThreshold > 0 AND quantity <= lowStockThreshold")
    fun lowStockCount(bid: Long): Flow<Int>

    @Query("SELECT COUNT(*) FROM products WHERE businessId = :bid")
